...
namespace ana{
  ...
  float GetCrossSectionScale( TH2D *unfold, TH1D *flux ){
    ...
    float cos_bins = unfold->GetXaxis()->GetBinWidth(1);
    float Tmu_bins = unfold->GetYaxis()->GetBinWidth(1); // GeV
    float flux_int = flux->Integral("width"); // Units of flux: #/m^2/50MeV/10^6 POT
    float Na       = 6.022e23;  //
    float M_fid    = 112000; // (kg / m^3) * m^3
    float A_ar     = 0.039948;  // kg / mol
    float tot_tgt  = ( Na * M_fid ) / ( A_ar ); // number of target particles  
    float POT      = 6.6e20;    // POT
    float scaling  = 5e8;      // Sort units from factor of 1 / ( integrated flux * Nt * bin widths * POT )
    float cm_conv  = 1e-38;     // take out factor of 10e-38 cm^2 
    float units_scale = 10e48; // Scale for units 

    return (scaling / ( cm_conv * cos_bins * Tmu_bins * flux_int * tot_tgt * POT ));
  ...
  void SetFluxStatsErrors( TH2D *unfold, TH2D *xsec, float norm ){
    ...
    for (int i = 0; i < n_y; ++i){
      for (int j = 0; j < n_x; ++j){
          float flux_error_squared = xsec->GetBinError(j, i);
          float statistical_error  = unfold->GetBinError(j, i);
          float total_error        = flux_error_squared + (statistical_error * statistical_error * norm * norm);
